...
while( row ~= -1 )
  ...
  if( row ~= -1 )
    ...
    elseif( ~isempty(strfind(row, 'Box_X') ) && section_simsys == 1)
       fprintf(fidout, [' Box_X=' num2str(input_struct.Box_X) ', Box_Y=' num2str(input_struct.Box_Y) ', Box_Z=' num2str(input_struct.Box_Z) ',' '\n']  );
    elseif( ~isempty(strfind(row, 'INDX') ) && section_simsys == 1)
       fprintf(fidout, [' INDX =  ' num2str(input_struct.INDX) ' , INDY = ' num2str(input_struct.INDY) ', INDZ = ' num2str(input_struct.INDZ) '\n']  );
    ...
    elseif( section_beam == 1)
       %for(n_beam=1:N_beams),
          ...
          fprintf(fidout, [' Charge = ' num2str(input_struct.charge, '%.1f') '\n']  );
          qp_par_mass_str = num2str(input_struct.mass, '%.3G');
          ...
          qp_gam_part_str = num2str(input_struct.gamma, '%.3G');
          ...
          qp_num_part_str = num2str(input_struct.N_particles, '%.3G');
          ...
          fprintf(fidout, [' Parameter_Array(1:1,1:3) = ' num2str(input_struct.beam_x_pos, '%.2f') ',' num2str(input_struct.beam_y_pos, '%.2f')  ',' num2str(input_struct.beam_z_pos, '%.2f') '\n']);
          fprintf(fidout, [' Parameter_Array(2:2,1:3) = ' num2str(input_struct.sigma_x, '%.2f') ',' num2str(input_struct.sigma_y, '%.2f')  ',' num2str(input_struct.sigma_z, '%.2f') '\n']);
          fprintf(fidout, [' Parameter_Array(3:3,1:3) = ' num2str(input_struct.emit_x, '%.3f') ',' num2str(input_struct.emit_y, '%.3f') ',' num2str(0., '%.3f')  '\n'] );
          ...
          fprintf(fidout, [' Destroyer_Criteria(2:2,1:5)=0,' num2str(input_struct.Box_X-2, '%.0f') ',0,' num2str(input_struct.Box_Y-2, '%.0f')  ',0' '\n']);
          fprintf(fidout, [' Destroyer_Criteria(3:3,1:5)=2,' num2str(input_struct.Box_X, '%.0f') ',2,' num2str(input_struct.Box_Y, '%.0f')  ',100' '\n']);
      ...
      elseif( ~isempty(strfind(row, 'Plasma_Density') ) && section_plasma == 1)
         Plasma_Density_str = num2str(input_struct.plasma_density, '%.3G');
      ...
      elseif( ~isempty(strfind(row, 'Nspecies') ) && section_plasma == 1)
         if( input_struct.plasma_PREION == 1)
      ...
      elseif( ~isempty(strfind(row, 'Neutral_gas') ) && section_neutral == 1)
         fprintf(fidout, [' Neutral_gas = ' num2str(input_struct.plasma_Z) '\n']  );

      % SPECIES SECTION 
      elseif( ~isempty(strfind(row, 'NP2') ) && section_species == 1)
         n_plasma_particles_per_cell = 4; % form UCLA
         fprintf(fidout, [' NP2 = ' num2str(round(sqrt((2^input_struct.INDX)^2 * n_plasma_particles_per_cell))) '\n']  );

      % SIMTIME SECTION
      elseif( ~isempty(strfind(row, 'TEND') ) && section_simtime == 1) 
         TEND_str = num2str(input_struct.TEND, '%10G');
         TEND_str = strrep(TEND_str,'E+0','E');
         TEND_str = strrep(TEND_str,'E+','E');
         fprintf(fidout, [' TEND =' TEND_str ', DT = ' num2str(input_struct.DT, '%.1f') '  ,' '\n']  );

      % DIAG SECTION
      elseif( ~isempty(strfind(row, 'DFESLICE') ) && section_fielddiag == 1) 
         fprintf(fidout, [' DFESLICE=' num2str(input_struct.DT_OUTPUT) ', EX0=0, EY0=' num2str(input_struct.beam_y_pos, '%.0f') ', EZ0=0' '\n']  );
      elseif( ~isempty(strfind(row, 'DFBSLICE') ) && section_fielddiag == 1) 
         fprintf(fidout, [' DFBSLICE=' num2str(input_struct.DT_OUTPUT) ', BX0=0, BY0=' num2str(input_struct.beam_y_pos, '%.0f') ', BZ0=0' '\n']  );
      elseif( ~isempty(strfind(row, 'DFQEB')) && isempty(strfind(row, 'DFQEBSLICE'))  && section_beamdiag == 1)
         if(input_struct.store_QEB_3D)
            fprintf(fidout, [' DFQEB=' num2str(input_struct.DT_OUTPUT) ',' '\n']  );
         else
            fprintf(fidout, [' DFQEB=0,' '\n']  );
         end% if
      elseif( ~isempty(strfind(row, 'DFQEBSLICE'))  && section_beamdiag == 1)
         fprintf(fidout, [' DFQEBSLICE=' num2str(input_struct.DT_OUTPUT) ' , QEBX0=0., QEBY0=' num2str(input_struct.beam_y_pos, '%.0f') ',  QEBZ0=0' '\n']  );
      elseif( ~isempty(strfind(row, 'DFQEPSLICE'))  && section_plasmadiag == 1)
         fprintf(fidout, [' DFQEPSLICE=' num2str(input_struct.DT_OUTPUT) ' , QEPX0= 0, QEPY0=' num2str(input_struct.beam_y_pos, '%.0f') ',  QEPZ0=0' '\n']  );
      elseif( ~isempty(strfind(row, 'DUMP_PHA_BEAM'))  && section_beamphasediag == 1)
         fprintf(fidout, [' DUMP_PHA_BEAM=.true., DFPHA_BEAM=' num2str(input_struct.DT_OUTPUT) ',' '\n']  );
      ...
      elseif( ~isempty(strfind(row, 'DUMP_RST_FILE'))  && section_restart == 1)
         if( input_struct.DT_RESTART)
            fprintf(fidout, [' DUMP_RST_FILE = .true.,  DFRST=' num2str(input_struct.DT_RESTART) '\n']  );
         else
