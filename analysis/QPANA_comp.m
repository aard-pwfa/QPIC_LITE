...
%rho_pad = N/(sqrt(2*pi)*sz)*exp(-zz.^2/(2*sz^2));
...
% max1_qp = max(ez_qp(zz<50));
% min1_qp = min(ez_qp(zz>50));
% max2_qp = max(ez_qp(zz>50));
% 
% max1_ana = max(ez_ana(zz<50));
% min1_ana = min(ez_ana(zz>50));
% max2_ana = max(ez_ana(zz>50));
...
%%
%figure(10);
%plot(zz, sum(beam_rho)/32,'b--',zz,E0*field_EZX(200,:),'b',zz,rho/1.8e14,'r--',1e6*zz_pad(good_ind)-5,Wz(good_ind)/1e9,'r');
%set(gca, 'xdir','reverse');
...
mb=max(abs(beam_rho(:)));
sb= mp/(2*mb);
imagesc(zz,xx,-sb*beam_rho/10-plas_rho); set(gca, 'xdir','reverse');
